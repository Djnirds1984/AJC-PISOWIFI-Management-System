...
const CoinModal = lazy(() => import('./CoinModal'));
...
// Optimized lightweight component for old devices
const LandingPageOptimized: React.FC<Props> = ({ rates, sessions, onSessionStart, refreshSessions, onRestoreSession }) => {
  ...
  useEffect(() => {
    ...
    // Load available slots with lower priority for low-end devices
  ...
  // Optimized audio playback - only on high-end devices
  useEffect(() => {
    if (mySession && mySession.remainingSeconds > 0 && config.connectedAudio && !isLowEndDevice) {
  ...
  }, [mySession, config.connectedAudio, isLowEndDevice]);

  const handleRefreshNetwork = async () => {
    setIsRefreshing(true);
    try {
      // Simple network refresh for low-end devices
      if (isLowEndDevice) {
        window.location.reload();
        return;
      }
      
      // Full network refresh for high-end devices
    ...
    <div className="portal-container min-h-screen" style={{ backgroundColor: config.backgroundColor, color: config.textColor }}>
      ...
      <main className="relative z-20">
        <div className="portal-card">
          ...
          {/* Conditionally render slot selection for low-end devices */}
          {availableSlots.length > 0 && !isLowEndDevice && (
        ...
        {/* Simplified pricing for low-end devices */}
        <div className="mb-10">
        ...
        {/* Conditionally render guide for low-end devices */}
        {!isLowEndDevice && (
      ...
      {showModal && (
        <Suspense fallback={<div className="fixed inset-0 bg-black/50 flex items-center justify-center"><div className="bg-white p-8 rounded-2xl">Loading...</div></div>}>
          <CoinModal 
