...
async function init() {
  ...
  // Vendors table for centralized machine management
  ...
  // Migration: Ensure vendors table exists (in case of older databases without it)
  try {
    await run(`CREATE TABLE IF NOT EXISTS vendors (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      hardware_id TEXT UNIQUE,
    ...
    const vendorCols = await all("PRAGMA table_info(vendors)");
    const colNames = vendorCols.map(col => col.name);
    
    // Add missing columns if they don't exist
    if (!colNames.includes('hardware_id')) {
      await run("ALTER TABLE vendors ADD COLUMN hardware_id TEXT UNIQUE");
    }
    if (!colNames.includes('license_key')) {
      await run("ALTER TABLE vendors ADD COLUMN license_key TEXT");
    }
    if (!colNames.includes('is_licensed')) {
      await run("ALTER TABLE vendors ADD COLUMN is_licensed INTEGER DEFAULT 0");
    }
    if (!colNames.includes('vendor_id')) {
      await run("ALTER TABLE vendors ADD COLUMN vendor_id TEXT");
    }
    if (!colNames.includes('centralized_key')) {
      await run("ALTER TABLE vendors ADD COLUMN centralized_key TEXT");
    }
    if (!colNames.includes('activated_at')) {
      await run("ALTER TABLE vendors ADD COLUMN activated_at DATETIME");
    }
    if (!colNames.includes('status')) {
      await run("ALTER TABLE vendors ADD COLUMN status TEXT DEFAULT 'offline'");
    }
    if (!colNames.includes('last_seen')) {
      await run("ALTER TABLE vendors ADD COLUMN last_seen DATETIME DEFAULT CURRENT_TIMESTAMP");
    }
    if (!colNames.includes('total_revenue')) {
      await run("ALTER TABLE vendors ADD COLUMN total_revenue DECIMAL(10, 2) DEFAULT 0.00");
    }
    if (!colNames.includes('coin_slot_pulses')) {
      await run("ALTER TABLE vendors ADD COLUMN coin_slot_pulses INTEGER DEFAULT 0");
    }
    if (!colNames.includes('created_at')) {
      await run("ALTER TABLE vendors ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP");
    }
  } catch (e) {
    console.error('[DB] Error creating vendors table:', e.message);
  }
