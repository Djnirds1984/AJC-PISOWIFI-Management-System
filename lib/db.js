...
async function init() {
  ...
  await run(`CREATE TABLE IF NOT EXISTS device_registry (
    mac TEXT PRIMARY KEY,
  ...
  await run(`CREATE INDEX IF NOT EXISTS idx_device_registry_hardware ON device_registry(hardware_signature)`);
  
  // Index for MAC lookups
  await run(`CREATE INDEX IF NOT EXISTS idx_device_registry_mac ON device_registry(mac)`);
