...
async function factoryResetDB() {
  ...
  const tables = ['rates', 'sessions', 'hotspots', 'wireless_settings', 'config', 'admin', 'admin_sessions'];
  ...
  await run(`INSERT INTO admin (username, password_hash, salt) VALUES (?, ?, ?)`, ['admin', hash, salt]);
...
db.serialize(() => {
  ...
  const { salt, hash } = hashPassword('admin');
  db.run(`INSERT OR IGNORE INTO admin (username, password_hash, salt) VALUES (?, ?, ?)`, ['admin', hash, salt]);
  ...
  db.run(`INSERT OR IGNORE INTO config (key, value) VALUES ('qos_discipline', 'cake')`);
});
