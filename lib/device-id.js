...
// Legacy function for backward compatibility - now just passes through headers
// since we're using Session ID system only, no device fingerprint needed
export function attachDeviceFingerprintHeaders(headers = {}) {
  // In Session ID system, we don't use device fingerprints
  // Just return headers as-is for compatibility
  return { ...headers };
}

// Legacy function for backward compatibility - now just attaches Session ID
export function attachDeviceHeaders(headers = {}) {
  return attachSessionHeaders(headers);
}
