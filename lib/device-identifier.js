...
class DeviceIdentifier {
   ...
   */
  static async updateSessionIdentification(sessionId, identifiers, sourceTable = 'sessions') {
    ...
    if (fingerprintIdentifier) {
      try {
        ...
        if (sourceTable === 'sessions') {
          // Local SQLite sessions table
          updateQuery = 'UPDATE sessions SET device_fingerprint = ? WHERE rowid = ?';
          queryParams = [fingerprintIdentifier.value, sessionId];
        } else {
          // Supabase/PostgreSQL clients table
          updateQuery = 'UPDATE clients SET device_fingerprint = ? WHERE id = ?';
          queryParams = [fingerprintIdentifier.value, sessionId];
        }
        
        await db.run(updateQuery, queryParams);
        console.log(`[Device-ID] Updated ${sourceTable} ${sessionId} with fingerprint`);
      } catch (e) {
