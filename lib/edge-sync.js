...
class EdgeSync {
  ...
  async registerOrFetchMachine() {
    ...
    try {
      ...
      let licenseKey = null;
      try {
        const licenseData = await db.get('SELECT license_key FROM license_info WHERE hardware_id = ?', [this.hardwareId]);
        if (licenseData && licenseData.license_key) {
          licenseKey = licenseData.license_key;
        }
      } catch (e) {
        console.warn('[EdgeSync] Failed to read local license key:', e.message);
      }

      // DEBUG: Introspect vendors table
      try {
          console.log('[EdgeSync] Introspecting vendors table...');
          const { data: introData, error: introError } = await this.supabase
            .from('vendors')
            .select('*')
            .limit(1);
          
          if (introData && introData.length > 0) {
              console.log('[EdgeSync] Machines table columns:', Object.keys(introData[0]));
          } else {
              console.log('[EdgeSync] Machines table empty or no access. Error:', introError);
          }
      } catch (e) {
          console.error('[EdgeSync] Introspection failed:', e);
      }

        // Check if machine exists
      const { data, error } = await this.supabase
        .from('vendors')
        .select('id, hardware_id') // Updated to match screenshot columns
        .eq('hardware_id', this.hardwareId)
        .maybeSingle(); // Use maybeSingle to avoid PGRST116 error handling
      ...
      */
      ...
      if (data) {
        ...
        // Update license key if we have one locally but maybe not in cloud
        if (licenseKey) {
            ...
            if (updateError) {
              console.error('[EdgeSync] Failed to update license key:', updateError);
            }
        }
        ...
        if (licenseKey) {
            newMachinePayload.license_key = licenseKey;
        }
  ...
  async syncMachineStatus(status) {
    ...
    try {
      ...
      const updatePayload = {
        // status, // Column not in screenshot
        last_seen: new Date().toISOString(),
        // cpu_temp: metrics.cpuTemp, // Column missing in machines table
        // uptime_seconds: metrics.uptime, // Column missing in machines table
        // active_sessions_count: metrics.activeSessions // Column missing in machines table
      };

      if (licenseKey) {
        updatePayload.license_key = licenseKey;
      }
  ...
  async recordSale(saleData) {
    ...
    try {
      // Fetch the vendor_id from the vendors table first
      ...
      if (vendorError || !vendorData || !vendorData.vendor_id) {
        console.warn('[EdgeSync] Cannot record sale - machine not claimed by vendor yet');
