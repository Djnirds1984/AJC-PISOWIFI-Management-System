...
class EdgeSync {
  ...
  async registerOrFetchMachine() {
    ...
    try {
      ...
      const { data, error } = await this.supabase
        .from('vendors')
        .select('id, vendor_id, hardware_id')
      ...
      if (data) {
        ...
        console.log(`[EdgeSync] Machine identified: ${this.machineId} (Vendor: ${this.vendorId || 'Pending'})`);
      } else {
        ...
        const { data: newData, error: insertError } = await this.supabase
          ...
          .insert({
            ...
            status: 'online',
            vendor_id: null // Explicitly null for pending activation
          })
          .select()
          .single();
  ...
  async recordSale(saleData) {
    if (!this.supabase || !this.vendorId) {
