...
class EdgeSync {
  ...
  async recordSale(saleData) {
    // TEMPORARY: Skip all sales to prevent queue buildup
    // Remove this when vendor is properly configured in Supabase
    console.log('[EdgeSync] Sales sync temporarily disabled - vendor not configured');
    return false;
    
    // If this sale is already marked as foreign key error, don't retry
    if (saleData._fkError) {
      console.warn('[EdgeSync] Skipping sale with foreign key error (already processed)');
      return false;
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '23503') { // foreign_key_violation
          console.warn(`[EdgeSync] Foreign key constraint error - vendor ${vendorId} may not exist in auth.users`);
          console.warn('[EdgeSync] This usually means the machine is not properly claimed by a vendor in Supabase');
          console.warn('[EdgeSync] Check that vendor_id exists in Supabase auth.users table');
          // Don't throw, just queue for retry with special marker
          const saleDataWithError = { ...saleData, _fkError: true };
          this.queueSync('sale', saleDataWithError);
          return false;
        }
        throw error;
      }
  ...
  queueSync(type, data) {
    const item = {
      ...
      retries: 0,
      vendorId: this.vendorId // Track which vendor this is for
    };
  ...
  async processQueue() {
    ...
    for (const item of itemsToProcess) {
      ...
      if (!success) {
        ...
        const isForeignKeyError = item.data && item.data._fkError;
        
        // Limit retries for foreign key errors (vendor doesn't exist)
        const maxRetries = isForeignKeyError ? 5 : 50;
        
        if (item.retries < maxRetries) {
            this.queue.push(item);
        } else {
          console.warn(`[EdgeSync] Giving up on ${item.type} after ${maxRetries} retries`);
          if (isForeignKeyError) {
            console.warn(`[EdgeSync] Vendor ${item.vendorId} likely doesn't exist in Supabase auth.users`);
            console.warn(`[EdgeSync] Manual intervention required - assign vendor in Supabase dashboard`);
          }
        }
      }
    }
