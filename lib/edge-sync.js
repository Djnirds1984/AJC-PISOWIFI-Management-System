...
class EdgeSync {
  ...
  async recordSale(saleData) {
    // DISABLED: Sales syncing disabled to prevent vendor_id errors
    console.log('[EdgeSync] Sales syncing disabled - skipping sale record');
    return true;
    ...
    try {
      ...
      // CRITICAL: Validate vendor_id is not null before attempting insert
      if (!vendorId) {
        console.warn('[EdgeSync] Cannot record sale - machine not claimed by vendor yet (vendor_id is null)');
        console.warn(`[EdgeSync] Machine ID: ${this.machineId} has no vendor assigned`);
        // Queue sale to try again later when vendor_id becomes available
        this.queueSync('sale', saleData);
        return false;
      }

      // Additional validation: Ensure vendor_id references a valid auth.users record
      // This prevents foreign key violations in vendor_dashboard_realtime view
      try {
        ...
        if (userCheckError || !userExists) {
          console.error(`[EdgeSync] Invalid vendor_id '${vendorId}' - does not exist in auth.users`);
          console.error('[EdgeSync] This will cause foreign key constraint violations');
          this.queueSync('sale', saleData);
          return false;
        }
      } catch (validationError) {
        // If we can't validate (due to permissions), log warning but proceed
        console.warn(`[EdgeSync] Could not validate vendor_id '${vendorId}' exists (permission denied)`);
        // Continue with the insert - the database will enforce the constraint if invalid
      }
      ...
      */
      ...
      if (err.message && err.message.includes('foreign key constraint')) {
        console.error('[EdgeSync] Foreign key constraint violation detected');
        console.error('[EdgeSync] This typically means vendor_id is invalid or references non-existent user');
        console.error(`[EdgeSync] Attempted vendor_id: ${this.vendorId || 'undefined'}`);
        console.error(`[EdgeSync] Machine ID: ${this.machineId}`);
        
        // Log the specific constraint that failed
        if (err.message.includes('vendor_dashboard_realtime')) {
          console.error('[EdgeSync] Constraint violated on vendor_dashboard_realtime view');
        } else if (err.message.includes('sales_logs')) {
          console.error('[EdgeSync] Constraint violated on sales_logs table');
        }
      }
   ...
   */
  queueSync(type, data) {
    // DISABLED: Queueing disabled to prevent vendor_id errors
    console.log(`[EdgeSync] Queueing disabled - skipping ${type}`);
    return;
   ...
   */
  async processQueue() {
    // DISABLED: Queue processing disabled to prevent vendor_id errors
    console.log('[EdgeSync] Queue processing disabled - clearing queue');
    this.queue = []; // Clear the queue
    this.saveQueue(); // Save empty queue
    return;
    
    if (this.queue.length === 0) return;

    console.log(`[EdgeSync] Processing queue: ${this.queue.length} items`);
    ...
    for (const item of itemsToProcess) {
      ...
      try {
        ...
        console.error(`[EdgeSync] Error processing queued ${item.type}:`, e.message);
      }
      ...
      if (success) {
        ...
        if (item.retries < 50) { // Max 50 retries
          ...
          console.error(`[EdgeSync] Dropping ${item.type} after 50 failed attempts`);
        }
      }
    }
    ...
    if (successCount > 0 || failureCount > 0) {
      console.log(`[EdgeSync] Queue processing complete: ${successCount} succeeded, ${failureCount} failed`);
      if (this.queue.length > 0) {
        console.log(`[EdgeSync] ${this.queue.length} items remain in queue for retry`);
      }
    }
  }
