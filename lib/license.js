...
class LicenseManager {
  ...
  async fetchAndCacheLicense(hardwareId) {
    ...
    try {
      ...
      if (!license) {
          ...
          // If we had a license locally but it's gone from cloud, it might be revoked
          const local = await db.get('SELECT * FROM license_info WHERE hardware_id = ?', [hardwareId]);
          if (local && local.license_key) {
            console.log(`[License] Local license ${local.license_key} not found in cloud. Marking as revoked.`);
  ...
  async verifyLicense() {
    ...
    try {
      ...
      if (!license) {
        // No license record at all in cloud for this hardware
        // Check if we have one locally that should be cleaned up
        const localLicense = await db.get('SELECT * FROM license_info WHERE hardware_id = ?', [hardwareId]);
        if (localLicense && localLicense.license_key) {
           console.log('[License] Local license exists but not found in cloud. Marking as revoked.');
           await db.run('UPDATE license_info SET is_active = 0, is_revoked = 1 WHERE hardware_id = ?', [hardwareId]);
           return { isValid: false, isActivated: false, isRevoked: true, error: 'License revoked by cloud' };
        }
