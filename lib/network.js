...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  // Upload Limiting (traffic sourced from the device)
  // Use iptables MARK to tag packets from device, then filter by mark
  if (uploadMbps > 0) {
    try {
      // Clear old marks first (both FORWARD and POSTROUTING)
      ...
      // Add new mark rule in POSTROUTING (egress/upload traffic)
      // POSTROUTING is where traffic goes just before leaving the interface
      await execPromise(`iptables -t mangle -I POSTROUTING 1 -s ${ip} -j MARK --set-mark ${markValue}`);
      
      // Create TC class and qdisc for upload (use replace if add fails)
      await execPromise(`tc class add dev ${lan} parent 1: classid ${ulHandle} htb rate ${uploadMbps}mbit ceil ${uploadMbps}mbit 2>/dev/null || tc class replace dev ${lan} parent 1: classid ${ulHandle} htb rate ${uploadMbps}mbit ceil ${uploadMbps}mbit`);
      
      // Remove old qdisc and add new one
      await execPromise(`tc qdisc del dev ${lan} parent ${ulHandle} 2>/dev/null || true`).catch(() => {});
      await new Promise(r => setTimeout(r, 10));
      await execPromise(`tc qdisc add dev ${lan} parent ${ulHandle} handle ${classId}1: ${discipline} bandwidth ${uploadMbps}mbit`);
      
      // Remove old mark filter at prio 100 (use different priority level to avoid conflicts with prio 1-2)
      await execPromise(`tc filter show dev ${lan} parent 1:0 prio 100 | grep -q "match mark" && tc filter del dev ${lan} parent 1:0 prio 100 2>/dev/null || true`).catch(() => {});
      await new Promise(r => setTimeout(r, 10));
      // Use u32 filter with mark matching at priority 100 to avoid conflicts with other filters
      await execPromise(`tc filter add dev ${lan} protocol ip parent 1:0 prio 100 u32 match mark ${markValue} 0xffffffff flowid ${ulHandle}`);
      
      console.log(`[QoS] Upload limit set for ${ip} using POSTROUTING mark and TC filter`);
    } catch (e) {
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    // Remove iptables mark rules (both FORWARD and POSTROUTING)
    await execPromise(`iptables -t mangle -D FORWARD -s ${ip} -j MARK --set-mark ${markValue} 2>/dev/null || true`).catch(() => {});
    await execPromise(`iptables -t mangle -D POSTROUTING -s ${ip} -j MARK --set-mark ${markValue} 2>/dev/null || true`).catch(() => {});
    ...
    // Remove upload filters and classes (using priority 100 for mark filter)
    await execPromise(`tc filter show dev ${lan} parent 1:0 prio 100 | grep -q "match mark" && tc filter del dev ${lan} parent 1:0 prio 100 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc qdisc del dev ${lan} parent ${ulHandle} 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc class del dev ${lan} parent 1: classid ${ulHandle} 2>/dev/null || true`).catch(() => {});
  } catch (e) {
