...
async function startPPPoEServer(config) {
  ...
  try {
    ...
    if (!isBridge) {
      console.log(`[PPPoE-Server] Flushing non-bridge interface ${targetIface}`);
      await execPromise(`ip addr flush dev ${targetIface}`).catch(() => {});
    ...
    // 6. Start pppoe-server daemon
    // -C: Service Name
    // -L: Local IP
    // -R: Start IP
    // -N: Number of sessions
    // -k: Kernel mode (if available)
    const serviceNameArg = service_name ? `-S "${service_name}" -C "${service_name}"` : '';
    const poolSize = parseInt(ip_pool_end.split('.')[3]) - parseInt(ip_pool_start.split('.')[3]) + 1;
    
    // Use nohup and redirect output to prevent hanging the exec call
    const cmd = `nohup pppoe-server -k -I ${targetIface} -L ${local_ip} -R ${ip_pool_start} -N ${poolSize} ${serviceNameArg} -O ${optionsPath} > /dev/null 2>&1 &`;
    
    console.log(`[PPPoE-Server] Executing: ${cmd}`);
    ...
    if (isRunning) {
      ...
      // 8. MSS Clamping for PPPoE (Crucial for bridge stability)
      ...
      throw new Error('Server failed to start. Check if rp-pppoe is installed and interface is valid.');
    }
