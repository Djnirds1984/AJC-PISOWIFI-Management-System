...
async function removeSpeedLimit(mac, ip) {
  if (!ip) return;
  try {
    const lan = await getLanInterface();
    ...
    // Delete Class (this automatically deletes the qdisc attached to it)
    await execPromise(`tc class del dev ${lan} parent 1: classid ${handle}`).catch(() => {});
    
  } catch (e) {
    // console.error(`[QoS] Remove Limit error:`, e.message);
  }
}

async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  if (!mac || !ip) return;
  console.log(`[QoS] Setting limit for ${mac} (${ip}): DL=${downloadMbps}M, UL=${uploadMbps}M`);
  
  try {
    const lan = await getLanInterface();
    const discipline = (await db.get("SELECT value FROM config WHERE key = 'qos_discipline'"))?.value || 'cake';
    
    // Use last octet of IP as unique class ID (assuming /24 subnet)
    const ipParts = ip.split('.');
    if (ipParts.length !== 4) return;
    const classId = parseInt(ipParts[3]);
    const handle = `1:${classId}`;
    
    // Clean up existing rules for this IP
    ...
    console.error(`[QoS] Set Limit error:`, e.message);
  }
}
