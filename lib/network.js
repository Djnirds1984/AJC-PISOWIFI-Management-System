...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  if (downloadMbps > 0) {
    try {
      ...
      await execPromise(`tc filter del dev ${lan} protocol ip parent 1:0 prio 1 u32 match ip dst ${ip} flowid ${dlHandle} 2>/dev/null || true`).catch(() => {});
  ...
  if (uploadMbps > 0) {
    try {
      ...
      await execPromise(`tc filter del dev ${lan} protocol ip parent 1:0 prio 2 fw 2>/dev/null || true`).catch(() => {});
      await execPromise(`tc filter add dev ${lan} protocol ip parent 1:0 prio 2 fw flowid ${ulHandle}`);
    } catch (e) {
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    // First, aggressively remove all related TC objects
    // Remove qdisc first (this will cascade remove filters and classes)
    await execPromise(`tc qdisc del dev ${lan} parent ${dlHandle} 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc qdisc del dev ${lan} parent ${ulHandle} 2>/dev/null || true`).catch(() => {});
