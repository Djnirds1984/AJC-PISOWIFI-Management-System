...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  if (uploadMbps > 0) {
    try {
      ...
      // Remove old fwmark filter more aggressively and add new one
      await execPromise(`tc filter show dev ${lan} parent 1:0 | grep -q "fw" && tc filter del dev ${lan} parent 1:0 prio 2 2>/dev/null || true`).catch(() => {});
      await new Promise(r => setTimeout(r, 10));
      await execPromise(`tc filter add dev ${lan} protocol ip parent 1:0 prio 2 fw classid ${ulHandle}`);
    } catch (e) {
