        ...
        if (stationsOutput) {
          ...
          for (const station of stations) {
            ...
            if (macMatch) {
              ...
              let ip = null;
              try {
                const { stdout: arpOutput } = await execPromise(`ip neigh show | grep ${mac}`).catch(() => ({ stdout: '' }));
              ...
              let hostname = '';
              try {
                const leaseFiles = ['/var/lib/dhcp/dhcpd.leases', '/var/lib/dnsmasq/dnsmasq.leases', '/tmp/dhcp.leases'];
                for (const leaseFile of leaseFiles) {
                  if (fs.existsSync(leaseFile)) {
                    const leaseContent = fs.readFileSync(leaseFile, 'utf8');
                    const leaseMatch = leaseContent.match(new RegExp(`${mac}.*?(\\S+)`, 'i'));
                    if (leaseMatch) {
                      hostname = leaseMatch[1];
                      break;
                    }
                  }
                }
