...
async function whitelistMAC(mac) {
  if (!mac) return;
  console.log(`[NET] Whitelisting MAC: ${mac}`);
  try {
    // Check if already whitelisted to avoid duplicates
    const { stdout } = await execPromise(`iptables -t nat -C PREROUTING -m mac --mac-source ${mac} -j ACCEPT`).catch(() => ({ stdout: '' }));
    if (!stdout) {
      // Insert at the top to bypass redirection
      await execPromise(`iptables -t nat -I PREROUTING 1 -m mac --mac-source ${mac} -j ACCEPT`);
      // Also allow in FORWARD chain for actual internet routing
...
async function blockMAC(mac) {
  if (!mac) return;
  console.log(`[NET] Blocking MAC: ${mac}`);
  try {
...
address=/#/${ip_address}
    ...
    // Clear only redirection rules, not user whitelists if possible
    // Note: We flush specifically to ensure a clean state for the portal
