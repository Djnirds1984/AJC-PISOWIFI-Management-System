...
async function setupCentralPortalIP() {
  try {
    console.log('[NET] Setting up centralized captive portal IP 10.0.0.1');
    
    // Create loopback alias for central portal
    await execPromise('ip addr add 10.0.0.1/32 dev lo').catch(() => {});
    
    // Enable IP forwarding
    await execPromise('sysctl -w net.ipv4.ip_forward=1').catch(() => {});
    
    // Configure routing for VLAN networks to reach central portal
    const vlanInterfaces = await db.all('SELECT name FROM vlans');
    for (const vlan of vlanInterfaces) {
      // Add route for VLAN subnet to use loopback
      const baseIP = vlan.name.split('.')[0]; // Extract parent interface
      if (baseIP) {
        // Assuming VLAN ID determines subnet (10.0.VLAN_ID.0/24)
        const vlanId = vlan.name.split('.').pop();
        if (vlanId && !isNaN(vlanId)) {
          const subnet = `10.0.${vlanId}.0/24`;
          await execPromise(`ip route add ${subnet} via 127.0.0.1 dev lo`).catch(() => {});
        }
      }
    }
    
    console.log('[NET] Central portal IP 10.0.0.1 configured');
  } catch (e) {
    console.error('[NET] Failed to setup central portal IP:', e.message);
  }
}
...
async function initFirewall() {
  ...
  try {
    // Setup centralized portal IP first
    await setupCentralPortalIP();
    ...
    // 5. Centralized Captive Portal Redirect (HTTP Port 80)
    // Redirect all non-authorized HTTP traffic to central portal at 10.0.0.1
    // This allows all VLANs to use the same portal server
    await execPromise(`iptables -t nat -A PREROUTING -i ${actualLan} -p tcp --dport 80 -j DNAT --to-destination 10.0.0.1:80`).catch(() => {});
    
    // Also redirect HTTPS to prevent bypass attempts
    await execPromise(`iptables -t nat -A PREROUTING -i ${actualLan} -p tcp --dport 443 -j DNAT --to-destination 10.0.0.1:80`).catch(() => {});
...
async function setupHotspot(config, skipRestart = false) {
  ...
  try {
    ...
    // Configure VLAN to use centralized captive portal at 10.0.0.1
    await execPromise(`iptables -t nat -A PREROUTING -i ${interface} -p tcp --dport 80 -j DNAT --to-destination 10.0.0.1:80`).catch(() => {});
    await execPromise(`iptables -t nat -A PREROUTING -i ${interface} -p tcp --dport 443 -j DNAT --to-destination 10.0.0.1:80`).catch(() => {});

    // Use bind-dynamic for better reliability on Linux with changing interfaces
    // Configure DNS to point to central portal at 10.0.0.1
      ...
      for (const vlan of vlanConfigs) {
        try {
          ...
          if (!existingVlan) {
            ...
            // Add route to central portal for this VLAN
            await execPromise(`ip route add ${prefix}.0/24 via 127.0.0.1 dev lo`).catch(() => {});
              
            console.log(`[NET] Configured ${vlan.name} with IP ${vlan.ip} as independent Hotspot segment routed to central portal.`);
          } else {
