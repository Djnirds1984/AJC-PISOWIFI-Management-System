...
async function cleanupAllNetworkSettings() {
  ...
  try {
    ...
    console.error('[NET] Cleanup Error:', e);
    throw e;
  }
}
...
async function setupHotspot(config) {
  ...
  try {
    ...
    // DNS Catch-all: address=/#/IP triggers the captive portal pop-up on mobile devices
    const dnsConfig = `
interface=${interface}
...
async function removeHotspot(interface) {
  try {
    ...
    await execPromise(`iptables -t nat -F`).catch(() => {}); // Simple flush for now
    await execPromise(`systemctl restart dnsmasq`);
  } catch (e) { throw e; }
}
