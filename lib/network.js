...
${password ? `wpa=2\nwpa_passphrase=${password}\nwpa_key_mgmt=WPA-PSK\nwpa_pairwise=TKIP\nrsn_pairwise=CCMP` : ''}
    ...
    // In real system, we might need to kill old hostapd or use multiple instances
    await execPromise(`systemctl stop hostapd || true`);
    // Note: This is simplified. Real multi-wifi needs separate systemd units per interface.
...
dhcp-option=6,8.8.8.8,8.8.4.4
    ...
    // Redirect to Port 3000 (Portal)
    await execPromise(`iptables -t nat -A PREROUTING -i ${interface} -p tcp --dport 80 -j REDIRECT --to-ports 3000`);
    await execPromise(`iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE`); // Default WAN is eth0
