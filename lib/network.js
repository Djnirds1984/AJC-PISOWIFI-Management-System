    ...
    if (isValidIp(ip)) {
      ...
      try {
        ...
        if (existingDevice) {
          ...
          // Insert new device with default limits
          const deviceId = `device_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
          await db.run(
            'INSERT INTO wifi_devices (id, mac, ip, download_limit, upload_limit, connected_at, last_seen, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [deviceId, mac, ip, dl, ul, Date.now(), Date.now(), 1]
          );
        }
      } catch (e) {
