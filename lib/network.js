...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    ...
    // Try multiple ways to delete the filter to ensure it's removed
    await execPromise(`tc filter del dev ${lan} parent ffff: protocol ip u32 match ip src ${ip} 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc filter del dev ${lan} parent ffff: prio 1 2>/dev/null || true`).catch(() => {});
    
    // Also try without specifying protocol to catch any variant
    await execPromise(`tc filter show dev ${lan} parent ffff: | grep "${ip}" && tc filter del dev ${lan} parent ffff: 2>/dev/null || true`).catch(() => {});
  } catch (e) {
