...
address=/#/${ip_address}
    ...
    // IP Tables:
    // 1. Allow traffic to the portal IP itself on Port 80 (Avoids redirection loops)
    await execPromise(`iptables -t nat -A PREROUTING -i ${interface} -d ${ip_address} -p tcp --dport 80 -j ACCEPT`).catch(() => {});
    
    // 2. Redirect all other Port 80 traffic to the Portal (Interception)
    await execPromise(`iptables -t nat -A PREROUTING -i ${interface} -p tcp --dport 80 -j REDIRECT --to-ports 80`);
    
    // 3. Masquerade for WAN access (eth0)
    await execPromise(`iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE`).catch(() => {}); 
    
    await execPromise(`systemctl restart dnsmasq`);
    console.log(`[HOTSPOT] Portal deployed on ${interface} at ${ip_address} (Interception Active on Port 80)`);
  } catch (e) { 
