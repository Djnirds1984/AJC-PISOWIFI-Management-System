...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  // Upload Limiting using IFB (Intermediate Functional Block)
  // IFB allows us to shape ingress traffic by redirecting to virtual device
  if (uploadMbps > 0) {
    try {
      const ifb = 'ifb0';
      ...
      // Initialize IFB qdisc if not already done
      try {
        await execPromise(`tc qdisc show dev ${ifb} | grep -q "htb"`);
      } catch (e) {
        console.log(`[QoS] Initializing HTB on IFB ${ifb}`);
        await execPromise(`tc qdisc del dev ${ifb} root`).catch(() => {});
        await execPromise(`tc qdisc add dev ${ifb} root handle 1: htb default 10`);
        await execPromise(`tc class add dev ${ifb} parent 1: classid 1:10 htb rate 1000mbit ceil 1000mbit`);
        await execPromise(`tc qdisc add dev ${ifb} parent 1:10 handle 10: ${discipline} bandwidth 1000mbit`);
      }
      
      // Create TC class and qdisc for upload on IFB
      await execPromise(`tc class add dev ${ifb} parent 1: classid ${ulHandle} htb rate ${uploadMbps}mbit ceil ${uploadMbps}mbit 2>/dev/null || tc class replace dev ${ifb} parent 1: classid ${ulHandle} htb rate ${uploadMbps}mbit ceil ${uploadMbps}mbit`);
      
      // Remove old qdisc and add new one
      await execPromise(`tc qdisc del dev ${ifb} parent ${ulHandle} 2>/dev/null || true`).catch(() => {});
      await new Promise(r => setTimeout(r, 20));
      await execPromise(`tc qdisc add dev ${ifb} parent ${ulHandle} handle ${classId}1: ${discipline} bandwidth ${uploadMbps}mbit`);
      
      // Ensure ingress qdisc exists on LAN interface
      try {
        await execPromise(`tc qdisc show dev ${lan} | grep -q "ingress"`) ;
      } catch (e) {
        console.log(`[QoS] Adding ingress qdisc to ${lan}`);
        await execPromise(`tc qdisc add dev ${lan} ingress`);
      }
      
      await new Promise(r => setTimeout(r, 20));
      
      // Delete ANY existing filters for this IP before adding new one
      // This is critical when MAC changes but IP stays the same
      await execPromise(`tc filter show dev ${lan} parent ffff: | grep "match ip src ${ip}" && (
        tc filter del dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} 2>/dev/null || \
        tc filter del dev ${lan} parent ffff: handle 0 u32 match ip src ${ip} 2>/dev/null || true
      ) || true`).catch(() => {});
      
      await new Promise(r => setTimeout(r, 20));
      
      // Add fresh ingress filter redirecting to IFB
      await execPromise(`tc filter add dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} flowid 1:${classId} action mirred egress redirect dev ${ifb}`);
      
      console.log(`[QoS] Upload limit set for ${ip}: Ingress filter on ${lan} redirects to ifb0 class ${ulHandle}`);
    } catch (e) {
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    // Remove iptables mark rules (cleanup old approach)
    await execPromise(`iptables -t mangle -D FORWARD -s ${ip} -j MARK --set-mark ${markValue} 2>/dev/null || true`).catch(() => {});
    await execPromise(`iptables -t mangle -D POSTROUTING -s ${ip} -j MARK --set-mark ${markValue} 2>/dev/null || true`).catch(() => {});
    
    // Remove download filters and classes from VLAN interface
    await execPromise(`tc filter show dev ${lan} parent 1:0 | grep -q "match ip dst ${ip}" && tc filter del dev ${lan} parent 1:0 protocol ip u32 match ip dst ${ip} 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc qdisc del dev ${lan} parent ${dlHandle} 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc class del dev ${lan} parent 1: classid ${dlHandle} 2>/dev/null || true`).catch(() => {});
    
    // Remove IFB ingress filter for this IP - try multiple approaches
    const ifb = 'ifb0';
    
    // Method 1: Delete by matching IP source
    await execPromise(`tc filter del dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} 2>/dev/null || true`).catch(() => {});
    
    // Method 2: Delete ALL ingress filters and recreate them (aggressive cleanup)
    // This ensures no stale filters remain for this IP
    await execPromise(`tc filter show dev ${lan} parent ffff: | grep -q "match ip src" && tc qdisc del dev ${lan} ingress && tc qdisc add dev ${lan} ingress 2>/dev/null || true`).catch(() => {});
    
    // Remove upload class from IFB
    await execPromise(`tc qdisc del dev ${ifb} parent ${ulHandle} 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc class del dev ${ifb} parent 1: classid ${ulHandle} 2>/dev/null || true`).catch(() => {});
    
    console.log(`[QoS] Cleanup complete for ${ip}`);
  } catch (e) {
