...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  if (uploadMbps > 0) {
    try {
      ...
      // Remove old ingress filter for this IP first
      ...
      const uploadKbps = uploadMbps * 1024; // Convert Mbps to Kbps
      const uploadBurst = uploadMbps * 128; // Burst size
      
      await execPromise(`tc filter add dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} police rate ${uploadMbps}mbit burst ${uploadBurst}k drop flowid :1`);
      
      console.log(`[QoS] Upload limit set for ${ip} using ingress police: ${uploadMbps}Mbps`);
    } catch (e) {
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    ...
    // Remove upload filter from ingress (use police action)
    await execPromise(`tc filter del dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} 2>/dev/null || true`).catch(() => {});
  } catch (e) {
