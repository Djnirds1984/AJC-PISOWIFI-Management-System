...
async function whitelistMAC(mac, ip) {
  ...
  try {
    ...
    if (isValidIp(ip)) {
      ...
      const device = await db.get('SELECT download_limit, upload_limit FROM wifi_devices WHERE mac = ? AND is_deleted = 0', [mac]);
      ...
      try {
        ...
        const existingDevice = await db.get('SELECT id, is_deleted FROM wifi_devices WHERE mac = ?', [mac]);
        const lanInterface = await getInterfaceForIp(ip) || 'unknown';
        
        if (existingDevice) {
          // If device was deleted, don't update it - respect the deletion
          if (existingDevice.is_deleted) {
            console.log(`[QoS] Skipping update of deleted device: ${mac}`);
            return;
          }
...
async function createVlan({ parent, id, name, autoConfigureWan = true }) {
  console.log(`[NET] Creating VLAN ${name} (ID: ${id}) on ${parent}`);
  
  // Generate proper VLAN interface name with length limit (max 15 chars)
  // Use shortened parent name if too long
  let shortParent = parent;
  if (parent.length > 10) {
    // For long interface names like enxb827eb0629bc, use first 10 chars
    shortParent = parent.substring(0, 10);
  }
  const vlanInterface = `${shortParent}.${id}`;
  
  // Ensure interface name doesn't exceed 15 characters
  if (vlanInterface.length > 15) {
    throw new Error(`VLAN interface name too long: ${vlanInterface} (${vlanInterface.length} chars, max 15)`);
  }
  
  try {
    await execPromise(`ip link add link ${parent} name ${vlanInterface} type vlan id ${id}`);
    await execPromise(`ip link set dev ${vlanInterface} up`);
    console.log(`[NET] VLAN interface ${vlanInterface} created successfully`);
  } catch (e) { 
    if (e.message.includes('File exists')) {
      // It's okay, just ensure it's up
      console.log(`[NET] VLAN interface ${vlanInterface} already exists, bringing it up`);
      await execPromise(`ip link set dev ${vlanInterface} up`).catch(() => {});
    } else {
      throw e; 
    }
  }

  // Auto-configure WAN VLANs with /20 network size
  if (autoConfigureWan) {
    try {
      // Detect if this is a WAN interface
      const interfaces = await getInterfaces();
      const { wanName } = classifyInterfaces(interfaces);
      
      if (parent === wanName) {
        console.log(`[NET] Detected WAN VLAN creation on ${parent}, auto-configuring with /20 network...`);
        
        // Generate /20 network based on VLAN ID
        // Use 10.{vlan_id}.0.0/20 format for WAN VLANs
        // This gives 4,094 hosts per VLAN (2^12 - 2)
        const networkBase = `10.${id}.0`;
        const gatewayIP = `${networkBase}.1`;
        const networkCIDR = `${networkBase}.0/20`;
        
        // Calculate DHCP range for /20 network
        // Start from .100 and use 80% of available range
        const dhcpStart = `${networkBase}.100`;
        const dhcpEnd = `10.${id}.15.254`; // End of /20 range
        const dhcpRange = `${dhcpStart},${dhcpEnd}`;
        
        console.log(`[NET] WAN VLAN ${name} network configuration:`, {
          network: networkCIDR,
          gateway: gatewayIP,
        ...
        // Flush any existing IP and set the gateway IP with /20 subnet
        await execPromise(`ip addr flush dev ${vlanInterface}`).catch(() => {});
        await execPromise(`ip addr add ${gatewayIP}/20 dev ${vlanInterface}`);
        
        // Ensure interface is up and independent (not bridged)
        await execPromise(`ip link set dev ${vlanInterface} up`);
        await execPromise(`ip link set dev ${vlanInterface} nomaster`).catch(() => {});
        
        // Auto-create hotspot configuration for this WAN VLAN
        try {
          await db.run(
            'INSERT OR REPLACE INTO hotspots (interface, ip_address, dhcp_range, enabled, bandwidth_limit) VALUES (?, ?, ?, 1, 100)',
            [vlanInterface, gatewayIP, dhcpRange]
          );
          console.log(`[NET] Auto-configured hotspot for WAN VLAN ${name} with /20 network`);
          
          // Setup the hotspot immediately
          await setupHotspot({
            interface: vlanInterface,
            ip_address: gatewayIP,
            dhcp_range: dhcpRange
          }, true); // Skip dnsmasq restart for now
          
        } catch (dbError) {
          console.error(`[NET] Failed to auto-configure hotspot for WAN VLAN ${name}:`, dbError.message);
        }
        
        console.log(`[NET] WAN VLAN ${name} auto-configured with /20 network (${networkCIDR})`);
      } else {
        console.log(`[NET] VLAN ${name} created on LAN interface ${parent}, skipping auto-configuration`);
      }
    } catch (autoConfigError) {
      console.error(`[NET] Auto-configuration failed for VLAN ${name}:`, autoConfigError.message);
      // Don't throw - VLAN creation was successful, just auto-config failed
    }
  }
}

async function deleteVlan(vlanRecord) {
  // Handle both old format (just name string) and new format (vlan object)
  let vlanName, vlanId, parentInterface;
  
  if (typeof vlanRecord === 'string') {
    // Old format - try to find the VLAN in database
    vlanName = vlanRecord;
    console.log(`[NET] Deleting VLAN ${vlanName} (legacy format)`);
    
    // Try to delete by name directly first (for backward compatibility)
    try {
      await execPromise(`ip link delete dev ${vlanName}`);
      return;
    } catch (e) {
      console.warn(`[NET] Could not delete VLAN by name ${vlanName}, trying database lookup`);
      
      // Fallback: Look up VLAN in database to get proper interface name
      try {
        const vlan = await db.get('SELECT * FROM vlans WHERE name = ?', [vlanName]);
        if (vlan) {
          // Generate shortened interface name
          let shortParent = vlan.parent;
          if (vlan.parent.length > 10) {
            shortParent = vlan.parent.substring(0, 10);
          }
          const vlanInterface = `${shortParent}.${vlan.id}`;
          console.log(`[NET] Found VLAN in database, deleting interface: ${vlanInterface}`);
          await execPromise(`ip link delete dev ${vlanInterface}`);
          return;
        } else {
          console.error(`[NET] VLAN ${vlanName} not found in database`);
          throw new Error(`VLAN ${vlanName} not found`);
        }
      } catch (dbError) {
        console.error(`[NET] Database lookup failed for VLAN ${vlanName}:`, dbError.message);
        throw dbError;
      }
    }
  } else {
    // New format - vlan object with parent, id, name
    vlanName = vlanRecord.name;
    vlanId = vlanRecord.id;
    parentInterface = vlanRecord.parent;
    
    // Generate shortened interface name
    let shortParent = parentInterface;
    if (parentInterface.length > 10) {
      shortParent = parentInterface.substring(0, 10);
    }
    const vlanInterface = `${shortParent}.${vlanId}`;
    
    console.log(`[NET] Deleting VLAN ${vlanName} (interface: ${vlanInterface})`);
    try {
      await execPromise(`ip link delete dev ${vlanInterface}`);
      return;
    } catch (e) {
      console.error(`[NET] Failed to delete VLAN interface ${vlanInterface}:`, e.message);
      throw e;
    }
  }
}
...
async function restartDnsmasq() {
  ...
  try {
    ...
    // 2. Restart the service with Orange Pi compatibility
    try {
      await execPromise('systemctl restart dnsmasq');
      console.log('[DNS] dnsmasq restarted successfully');
    } catch (restartError) {
      console.error('[DNS] systemctl restart failed, trying alternative methods...');
      
      // Alternative restart method for Orange Pi
      try {
        await execPromise('sudo killall dnsmasq 2>/dev/null || true');
        await new Promise(r => setTimeout(r, 1000)); // Wait 1 second
        await execPromise('sudo systemctl start dnsmasq');
        console.log('[DNS] dnsmasq restarted using alternative method');
      } catch (altError) {
        console.error('[DNS] Alternative restart also failed:', altError.message);
        
        // Final fallback - just start dnsmasq directly
        try {
          await execPromise('sudo dnsmasq --conf-file=/etc/dnsmasq.conf');
          console.log('[DNS] dnsmasq started directly as fallback');
        } catch (directError) {
          console.error('[DNS] All restart methods failed. Hotspot may not work properly.');
          // Don't throw error - allow hotspot creation to continue
          // The network interface and IP configuration will still work
          return;
        }
      }
    }
    ...
    // Don't throw error on Orange Pi - allow hotspot creation to continue
    console.log('[DNS] Continuing hotspot creation despite dnsmasq restart failure...');
  }
}

// Get the appropriate dnsmasq config directory based on board type
function getDnsmasqConfigDir() {
  // Check if we're on Orange Pi or similar ARM board
  try {
    const fs = require('fs');
    if (fs.existsSync('/proc/device-tree/model')) {
      const model = fs.readFileSync('/proc/device-tree/model', 'utf8').toLowerCase();
      if (model.includes('orange') || model.includes('pi')) {
        // Use RAM-based directory for Orange Pi to avoid filesystem issues
        return '/run/dnsmasq.d';
      }
    }
  } catch (e) {
    // Fallback detection
  ...
  // Default to /etc for x64 systems
...
async function setupHotspot(config, skipRestart = false) {
  ...
  try {
    ...
    console.log(`[HOTSPOT] Configured ${interface} with /${bitmask} network (${ip_address}/${bitmask})`);
...
address=/#/${ip_address}`;
    ...
    // Use appropriate config directory based on board type
    const configDir = getDnsmasqConfigDir();
    const configFile = `${configDir}/ajc_${interface}.conf`;
    
    console.log(`[HOTSPOT] Writing dnsmasq config to: ${configFile}`);
    ...
    fs.writeFileSync(configFile, dnsConfig);
    
    if (!skipRestart) {
      await restartDnsmasq();
    }
    console.log(`[HOTSPOT] Segment Live on ${interface} with /${bitmask} network`);
  } catch (e) { throw e; }
}
...
async function removeHotspot(interface, skipRestart = false) {
  try {
    ...
    const filesToCheck = [
      `${configDir}/ajc_${targetInterface}.conf`,
      `${configDir}/ajc_${interface}.conf`
    ];
    
    console.log(`[HOTSPOT] Removing config files from: ${configDir}`);
    
    for (const file of filesToCheck) {
      if (fs.existsSync(file)) {
        console.log(`[HOTSPOT] Removing config file: ${file}`);
        fs.unlinkSync(file);
      }
    }
...
async function autoProvisionNetwork() {
  ...
  try {
    ...
    // --- Auto-Configure VLANs on WAN with /20 Networks ---
    if (wanName) {
      console.log(`[NET] Checking VLAN configuration for WAN (${wanName}) with /20 networks...`);
      const vlanConfigs = [
        { id: 13, name: `${wanName}.13` },
        { id: 22, name: `${wanName}.22` }
      ];
      
      for (const vlan of vlanConfigs) {
        try {
          // Check if VLAN exists in DB first
          const existingVlan = await db.get('SELECT * FROM vlans WHERE name = ?', [vlan.name]);
          
          if (!existingVlan) {
            console.log(`[NET] Auto-provisioning missing WAN VLAN: ${vlan.name} with /20 network`);
            
            // Create VLAN with auto-configuration disabled to avoid double-configuration
            await createVlan({ parent: wanName, id: vlan.id, name: vlan.name, autoConfigureWan: false }).catch(() => {});
            
            // Generate shortened VLAN interface name
            let shortParent = wanName;
            if (wanName.length > 10) {
              shortParent = wanName.substring(0, 10);
            }
            const vlanInterface = `${shortParent}.${vlan.id}`;
            
            // Configure /20 network manually for auto-provisioning
            const networkBase = `10.${vlan.id}.0`;
            const gatewayIP = `${networkBase}.1`;
            const dhcpStart = `${networkBase}.100`;
            const dhcpEnd = `10.${vlan.id}.15.254`; // End of /20 range
            const dhcpRange = `${dhcpStart},${dhcpEnd}`;
            
            // Set IP with /20 subnet
            await execPromise(`ip addr flush dev ${vlanInterface}`);
            await execPromise(`ip addr add ${gatewayIP}/20 dev ${vlanInterface}`);
            ...
            // Configure as independent Hotspot Segment with /20 range
            await db.run('INSERT OR REPLACE INTO hotspots (interface, ip_address, dhcp_range, enabled, bandwidth_limit) VALUES (?, ?, ?, 1, 100)', 
              [vlanInterface, gatewayIP, dhcpRange]);
              
            console.log(`[NET] Configured ${vlan.name} with /20 network (10.${vlan.id}.0.0/20) - 4,094 hosts capacity`);
          } else {
             console.log(`[NET] VLAN ${vlan.name} already configured in DB. Skipping auto-provision.`);
          }
        } catch (e) {
...
async function initQoS(interface, discipline = 'cake') {
  ...
  try {
    ...
    // Get max bandwidth from config, default to 10000mbit (10G) for 10G+ support
    ...
    await execPromise(`tc class add dev ${interface} parent 1: classid 1:10 htb rate ${maxBandwidth}mbit ceil ${maxBandwidth}mbit`);
    
    // Add qdisc for default class
    await execPromise(`tc qdisc add dev ${interface} parent 1:10 handle 10: ${discipline} bandwidth ${maxBandwidth}mbit`);
    
    console.log(`[QoS] ${discipline.toUpperCase()} successfully applied on ${interface} with max bandwidth ${maxBandwidth}mbit`);
    ...
    try {
      ...
      if (gamingEnabled?.value === '1') {
        await applyGamingPriority(interface, true, parseInt(gamingPercentage?.value || '20'), maxBandwidth);
      }
    } catch (e) {
...
async function applyGamingPriority(interface, enabled, percentage, maxBandwidth = 1000) {
  if (!interface) return;
...
async function getPPPoESessions() {
  try {
    ...
    for (const pppIface of pppInterfaces) {
      ...
      const ip = pppIface.addr_info?.[0]?.local || 'N/A';
...
module.exports = { 
  ...
  forceNetworkRefresh: async (mac, ip) => {
    ...
    try {
      // 1. Clear all connection tracking for this IP/MAC
      await execPromise(`conntrack -D -s ${ip} 2>/dev/null || true`).catch(() => {});
      await execPromise(`conntrack -D -d ${ip} 2>/dev/null || true`).catch(() => {});
      await execPromise(`conntrack -D -p tcp -s ${ip} 2>/dev/null || true`).catch(() => {});
      await execPromise(`conntrack -D -p udp -s ${ip} 2>/dev/null || true`).catch(() => {});
      
      // 2. Send gratuitous ARP to refresh network cache
      try {
        const interfaceCmd = await execPromise(`ip route get ${ip} | grep -o 'dev [^ ]*' | cut -d' ' -f2`);
        const iface = interfaceCmd.stdout.trim();
        if (iface) {
          await execPromise(`arping -c 1 -U -I ${iface} ${ip} 2>/dev/null || true`).catch(() => {});
        }
      } catch (e) {
        // arping might not be available, that's fine
      }
      
      // 3. Re-apply whitelist rules
      await whitelistMAC(mac, ip);
      
      // 4. Ping to trigger network stack
      await execPromise(`ping -c 1 -W 1 ${ip} 2>/dev/null || true`).catch(() => {});
      
      // 5. Small delay to let network settle
      await new Promise(r => setTimeout(r, 200));
  ...
  cleanupAllNetworkSettings: async () => {
    ...
    // Clean up configs from appropriate directory based on board type
    const configDir = getDnsmasqConfigDir();
    await execPromise(`rm -f ${configDir}/ajc_*.conf`);
  ...
  async validateIpRange(startIp, endIp) {
    try {
      const start = ipToInt(startIp);
      const end = ipToInt(endIp);
      return start <= end && end - start < 65536; // Max 65536 IPs per pool
    } catch (e) {
      return false;
    }
  },
  
  async checkIpOverlap(startIp, endIp, excludePoolId = null) {
    const db = require('./db');
    let query = `
      SELECT id, name FROM ip_pools 
      WHERE (start_ip BETWEEN ? AND ? OR end_ip BETWEEN ? AND ?)
      AND status = 'active'
    `;
    let params = [startIp, endIp, startIp, endIp];
  ...
  async assignIpPool(poolId, assignedTo, assignedType) {
    const db = require('./db');
    await db.run(
      'UPDATE ip_pools SET assigned_to = ?, assigned_type = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [assignedTo, assignedType, poolId]
    );
  },
  
  async unassignIpPool(poolId) {
    const db = require('./db');
    await db.run(
  ...
  async getPoolUtilization(poolId) {
    const db = require('./db');
    const pool = await db.get('SELECT start_ip, end_ip FROM ip_pools WHERE id = ?', [poolId]);
    if (!pool) return null;
  ...
  async getAllPoolUtilizations() {
    const db = require('./db');
    const pools = await db.all('SELECT id, start_ip, end_ip FROM ip_pools WHERE status = "active"');
