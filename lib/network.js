...
async function initFirewall() {
  ...
  try {
    ...
    // 4.1 Allow PPPoE traffic to WAN
    await execPromise(`iptables -A FORWARD -i ppp+ -o ${wan} -j ACCEPT`).catch(() => {});
    // Allow WAN to PPPoE (Established) is already handled by rule above
    
    // 4.2 MSS Clamping for PPPoE (Crucial for browsing)
...
async function startPPPoEServer(config) {
  ...
  try {
    ...
    // We use the full path to pppoe-server if possible, and ensure no hidden flags are used
    const cmd = `nohup pppoe-server -d -I ${targetIface} -L ${local_ip} -R ${ip_pool_start} -N ${poolSize} ${serviceNameArg} -O ${optionsPath} >> /var/log/pppoe-server.log 2>&1 &`;
    ...
    if (isRunning) {
      ...
      // 9. Save to database
      await db.run(
        'INSERT OR REPLACE INTO pppoe_server (id, interface, local_ip, ip_pool_start, ip_pool_end, dns1, dns2, service_name, enabled) VALUES (1, ?, ?, ?, ?, ?, ?, ?, 1)',
...
async function getPPPoEServerStatus() {
  try {
    const config = await db.get('SELECT * FROM pppoe_server ORDER BY id DESC LIMIT 1');
