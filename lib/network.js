...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  if (uploadMbps > 0) {
    try {
      ...
      // Set up ingress qdisc on the LAN interface to redirect to IFB
      // Only add if it doesn't exist
      try {
        await execPromise(`tc qdisc show dev ${lan} | grep -q "ingress"`).catch(async () => {
      ...
      // Remove old ingress filter for this IP
      await execPromise(`tc filter del dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} 2>/dev/null || true`).catch(() => {});
      
      await new Promise(r => setTimeout(r, 10));
      
      // Add filter to redirect traffic from this IP to IFB with proper flowid mapping
      // Traffic matching this filter will be mirrored to ifb0 and needs to reach the right class
      ...
      console.log(`[QoS] Upload limit set for ${ip}: Ingress filter redirects to ifb0 class ${ulHandle}`);
    } catch (e) {
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    ...
    // Remove IFB ingress filter for this IP
    const ifb = 'ifb0';
    await execPromise(`tc filter del dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} 2>/dev/null || true`).catch(() => {});
    
    // Only delete ingress qdisc if no other filters exist (optional - keep it for other IPs)
    // await execPromise(`tc qdisc del dev ${lan} ingress 2>/dev/null || true`).catch(() => {});
