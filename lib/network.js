...
address=/#/${ip_address}
    ...
    // IP TABLES FLUSH FOR THIS INTERFACE
    await execPromise(`iptables -t nat -D PREROUTING -i ${interface} -j REDIRECT --to-ports 80`).catch(() => {});

    // 1. ALLOW DNS (Port 53) - CRITICAL for browser to even "try" a URL
    ...
    // 2. ALLOW traffic to the gateway itself (Portal Assets)
