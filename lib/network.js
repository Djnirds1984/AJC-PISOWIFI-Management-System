...
async function whitelistMAC(mac, ip) {
  ...
  try {
    ...
    if (isValidIp(ip)) {
      ...
      // Apply Speed Limit if session exists
      const session = await db.get('SELECT download_limit, upload_limit FROM sessions WHERE mac = ?', [mac]);
      if (session && (session.download_limit > 0 || session.upload_limit > 0)) {
        await setSpeedLimit(mac, ip, session.download_limit, session.upload_limit);
      }
    }
  } catch (e) {
