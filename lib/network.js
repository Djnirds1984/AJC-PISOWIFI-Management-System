...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  const dlHandle = `1:${classId}`;
  const ulHandle = `1:${classId + 200}`; // Separate class ID under same root for upload traffic
  ...
  if (downloadMbps > 0) {
    ...
    try {
      ...
      await execPromise(`tc qdisc add dev ${lan} parent ${dlHandle} handle ${classId}: ${discipline} bandwidth ${downloadMbps}mbit`);
  ...
  if (uploadMbps > 0) {
    ...
    try {
      const ulClassId = classId + 200; // Offset to avoid collision with download classes
      await execPromise(`tc class add dev ${lan} parent 1: classid ${ulHandle} htb rate ${uploadMbps}mbit ceil ${uploadMbps}mbit`);
      await execPromise(`tc qdisc add dev ${lan} parent ${ulHandle} handle ${ulClassId}: ${discipline} bandwidth ${uploadMbps}mbit`);
...
async function removeSpeedLimit(mac, ip) {
  ...
  const classId = parseInt(ipParts[3]);
  const dlHandle = `1:${classId}`;
  const ulHandle = `1:${classId + 200}`; // Offset for upload class
  ...
  try {
    ...
    await execPromise(`tc qdisc del dev ${lan} parent ${dlHandle} handle ${classId}:`).catch(() => {});
    const ulClassId = classId + 200;
    await execPromise(`tc qdisc del dev ${lan} parent ${ulHandle} handle ${ulClassId}:`).catch(() => {});
