...
function startLogTailing() {
  ...
  try {
    ...
    // Tail both pppd.log and pppoe-server.log
...
async function startPPPoEServer(config) {
  ...
  try {
    ...
    try {
      if (!fs.existsSync('/var/log/pppd.log')) {
        fs.writeFileSync('/var/log/pppd.log', '');
      }
      await execPromise('chmod 666 /var/log/pppd.log').catch(() => {});
    } catch (e) {}
    ...
    // Use nohup and redirect output to /var/log/pppoe-server.log
    // Note: Removed -k flag as it's not supported by this pppoe-server version
    const cmd = `nohup pppoe-server -I ${targetIface} -L ${local_ip} -R ${ip_pool_start} -N ${poolSize} ${serviceNameArg} -O ${optionsPath} >> /var/log/pppoe-server.log 2>&1 &`;
