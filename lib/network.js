...
async function createVlan({ parent, id, name }) {
  ...
  // Generate proper VLAN interface name: parent.vlanid (e.g., eth0.100)
  const vlanInterface = `${parent}.${id}`;
  ...
  if (typeof vlanRecord === 'string') {
    ...
    try {
      ...
      try {
        ...
        if (vlan) {
          const vlanInterface = `${vlan.parent}.${vlan.id}`;
    ...
    // Generate proper VLAN interface name: parent.vlanid (e.g., eth0.100)
    const vlanInterface = `${parentInterface}.${vlanId}`;
      ...
      for (const vlan of vlanConfigs) {
        try {
          ...
          if (!existingVlan) {
            ...
            // Generate proper VLAN interface name: parent.vlanid (e.g., eth0.13)
            const vlanInterface = `${wanName}.${vlan.id}`;
