...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  const markId = classId; // Use IP octet as iptables mark (1-254)
  ...
  if (uploadMbps > 0) {
    try {
      // Mark packets from this IP with iptables (use OUTPUT chain for egress)
      // Mark value: 0x{classId}00 (e.g., 0xa000 for IP .160)
      ...
      // Create TC class and qdisc for upload
      ...
      // Filter by fwmark (iptables mark)
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    ...
    await execPromise(`tc filter del dev ${lan} protocol ip parent 1:0 prio 2 fw classid ${ulHandle}`).catch(() => {});
