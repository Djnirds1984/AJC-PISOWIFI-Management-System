    ...
    if (wanName) {
      ...
      for (const vlan of vlanConfigs) {
        try {
          ...
          // Add to LAN members for bridging
          if (!lanMembers.includes(vlan.name)) {
            lanMembers.push(vlan.name);
          }
          ...
          console.log(`[NET] Configured ${vlan.name} with IP ${vlan.ip} and added to bridge pool.`);
        } catch (e) {
