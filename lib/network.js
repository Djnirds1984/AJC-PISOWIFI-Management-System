...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  ...
  // IFB allows us to shape ingress traffic
  if (uploadMbps > 0) {
    try {
      const ifb = 'ifb0';
      ...
      // Add filter to redirect traffic from this IP to IFB
      try {
        await execPromise(`tc filter del dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} 2>/dev/null || true`).catch(() => {});
      } catch (e) {}
      
      await new Promise(r => setTimeout(r, 10));
      await execPromise(`tc filter add dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} flowid ${ulHandle} action mirred egress redirect dev ${ifb}`);
      
      console.log(`[QoS] Upload limit set for ${ip} using IFB ${ifb} and ingress shaping`);
    } catch (e) {
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    ...
    // Remove IFB ingress rules and upload classes
    const ifb = 'ifb0';
    await execPromise(`tc filter del dev ${lan} parent ffff: protocol ip prio 1 u32 match ip src ${ip} 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc qdisc del dev ${lan} ingress 2>/dev/null || true`).catch(() => {});
    await execPromise(`tc qdisc del dev ${ifb} parent ${ulHandle} 2>/dev/null || true`).catch(() => {});
