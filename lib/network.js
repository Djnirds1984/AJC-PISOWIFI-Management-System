...
async function setSpeedLimit(mac, ip, downloadMbps, uploadMbps) {
  if (!mac || !ip) return;
  console.log(`[DEBUG] setSpeedLimit called: mac=${mac}, ip=${ip}, DL=${downloadMbps}, UL=${uploadMbps}`);
  ...
  const handle = `1:${classId}`;
  const ulHandle = `2:${classId}`; // Separate handle for upload traffic
  ...
  if (downloadMbps > 0) {
    ...
    try {
      await execPromise(`tc class add dev ${lan} parent 1: classid ${handle} htb rate ${downloadMbps}mbit ceil ${downloadMbps}mbit`);
      await execPromise(`tc qdisc add dev ${lan} parent ${handle} handle ${classId}: ${discipline} bandwidth ${downloadMbps}mbit`);
      await execPromise(`tc filter add dev ${lan} protocol ip parent 1:0 prio 1 u32 match ip dst ${ip} flowid ${handle}`);
    } catch (e) {
  ...
  if (uploadMbps > 0) {
    ...
    try {
      ...
      await execPromise(`tc qdisc add dev ${lan} parent ${ulHandle} handle ${classId}: ${discipline} bandwidth ${uploadMbps}mbit`);
...
async function removeSpeedLimit(mac, ip) {
  ...
  try {
    ...
    await execPromise(`tc filter del dev ${lan} protocol ip parent 1:0 prio 1 u32 match ip dst ${ip} flowid ${handle}`).catch(() => {});
    ...
    // Remove both qdiscs (if they exist)
    await execPromise(`tc qdisc del dev ${lan} parent ${handle} handle ${classId}:`).catch(() => {});
    await execPromise(`tc qdisc del dev ${lan} parent ${ulHandle} handle ${classId}:`).catch(() => {});
    
    // Remove both classes (if they exist)
    await execPromise(`tc class del dev ${lan} parent 1: classid ${handle}`).catch(() => {});
...
async function whitelistMAC(mac, ip) {
  ...
  try {
    ...
    if (isValidIp(ip)) {
      ...
      // Check device limits first - but only use them if they're explicitly set (non-zero)
      if (device) {
        if (device.download_limit > 0) dl = device.download_limit;
        if (device.upload_limit > 0) ul = device.upload_limit;
      }
      
      // Fall back to session limits if device limits aren't set
      if (session) {
        if (dl === 0 && session.download_limit > 0) dl = session.download_limit;
        if (ul === 0 && session.upload_limit > 0) ul = session.upload_limit;
      }
