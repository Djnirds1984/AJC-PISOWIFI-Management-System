...
async function kickStation(mac) {
  if (!mac) return;
  console.log(`[NET] Kicking station: ${mac}`);
  try {
    const ifaces = await getInterfaces();
    const wifiIface = ifaces.find(i => i.type === 'wifi' && i.status === 'up')?.name || 'wlan0';

    // 1. Attempt to deauthenticate using hostapd_cli
    // We try both with and without the interface flag
    await execPromise(`hostapd_cli -i ${wifiIface} deauthenticate ${mac}`).catch(() => {});
    await execPromise(`hostapd_cli -i ${wifiIface} disassociate ${mac}`).catch(() => {});
    await execPromise(`hostapd_cli deauthenticate ${mac}`).catch(() => {});
    
    // 2. Fallback: use iw to delete the station from the kernel
    await execPromise(`iw dev ${wifiIface} station del ${mac}`).catch(() => {});
    
    // 3. Last resort: disconnect using nmcli if available
    await execPromise(`nmcli device wifi rescan`).catch(() => {});
  } catch (e) {
    console.log(`[NET] Kick station ${mac} failed (non-critical):`, e.message);
  }
}

async function whitelistMAC(mac, ip) {
  if (!mac) return;
  console.log(`[NET] Aggressively Authorizing Client: ${mac} (${ip || 'no-ip'})`);
  try {
    ...
    // 2. Clear any PREROUTING redirect for this MAC (so they don't get sent to portal)
    await execPromise(`iptables -t nat -I PREROUTING 1 -m mac --mac-source ${mac} ${ipFilter} -j ACCEPT`).catch(() => {});

    // 3. Kick the station to force a re-connect and refresh their network state
    await kickStation(mac);
    
    // 4. Clear conntrack entries to ensure no "leaked" state
    if (isValidIp(ip)) {
...
async function blockMAC(mac, ip) {
  if (!mac) return;
  console.log(`[NET] Aggressively Blocking Client: ${mac} (${ip || 'no-ip'})`);
  try {
    const ipFilter = isValidIp(ip) ? `-s ${ip}` : '';
    
    // 1. Remove from FORWARD and PREROUTING
    ...
    // 2. Explicitly REJECT their traffic (at the top) to kill active sessions immediately
    await execPromise(`iptables -I FORWARD 1 -m mac --mac-source ${mac} ${ipFilter} -j REJECT`).catch(() => {});
    
    // 3. Clear conntrack entries
    if (isValidIp(ip)) {
      await execPromise(`conntrack -D -s ${ip} 2>/dev/null || true`).catch(() => {});
      await execPromise(`conntrack -D -d ${ip} 2>/dev/null || true`).catch(() => {});
    }

    // 4. Kick the station to drop their current connection
    await kickStation(mac);
  } catch (e) {
