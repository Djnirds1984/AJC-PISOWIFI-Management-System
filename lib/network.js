...
async function whitelistMAC(mac, ip) {
  ...
  try {
    ...
    if (isValidIp(ip)) {
      ...
      // Check both independently so each can fall back to session separately
      ...
      // Download: Use device limit if set, otherwise use session limit
      ...
      // Upload: Use device limit if set, otherwise use session limit
      ...
      try {
        await db.run(
          'UPDATE wifi_devices SET download_limit = ?, upload_limit = ? WHERE mac = ?',
          [dl, ul, mac]
        );
      } catch (e) {
...
async function startPPPoEServer(config) {
  const { interface: iface, local_ip, ip_pool_start, ip_pool_end, dns1 = '8.8.8.8', dns2 = '8.8.4.4', service_name = '' } = config;
  
  console.log(`[PPPoE-Server] Starting PPPoE server on ${iface}...`);
  
  try {
    // 1. Stop any existing PPPoE server
    await stopPPPoEServer(iface);
    
    // 2. Ensure interface is up
    await execPromise(`ip link set ${iface} up`);
    await execPromise(`ip addr flush dev ${iface}`);
    await execPromise(`ip addr add ${local_ip}/24 dev ${iface}`);
    
    // 3. Create pppoe-server configuration
    const configDir = '/etc/ppp';
    if (!fs.existsSync(configDir)) {
      fs.mkdirSync(configDir, { recursive: true });
    }
    
    // 4. Create options file for pppoe-server
    ...
    console.log(`[PPPoE-Server] Options file created`);
    
    // 5. Start pppoe-server daemon
    const serviceNameArg = service_name ? `-S ${service_name}` : '';
    const cmd = `pppoe-server -I ${iface} -L ${local_ip} -R ${ip_pool_start} -N ${parseInt(ip_pool_end.split('.')[3]) - parseInt(ip_pool_start.split('.')[3]) + 1} ${serviceNameArg} -O ${optionsPath}`;
    ...
    if (isRunning) {
      console.log(`[PPPoE-Server] Server started successfully on ${iface}`);
      
      // 7. Save to database
      await db.run(
        'INSERT OR REPLACE INTO pppoe_server (interface, local_ip, ip_pool_start, ip_pool_end, dns1, dns2, service_name, enabled) VALUES (?, ?, ?, ?, ?, ?, ?, 1)',
        [iface, local_ip, ip_pool_start, ip_pool_end, dns1, dns2, service_name]
      );
      
      return { success: true, message: 'PPPoE Server started' };
    } else {
      throw new Error('Server failed to start - check pppoe-server installation');
    }
...
async function stopPPPoEServer(iface) {
  ...
  try {
    // Kill pppoe-server process
    await execPromise(`killall pppoe-server`).catch(() => {});
    
    // Kill any pppd processes
    await execPromise(`killall pppd`).catch(() => {});
...
async function getPPPoEServerStatus() {
  try {
    ...
    if (!config) {
      return {
        running: false,
        message: 'No PPPoE server configured'
      };
    }
    ...
    if (!running) {
      return {
        running: false,
        config,
        message: 'Server configured but not running'
      };
    }
...
async function addPPPoEUser(username, password) {
  console.log(`[PPPoE-Server] Adding user: ${username}`);
  
  try {
    // 1. Add to database
    await db.run(
      'INSERT INTO pppoe_users (username, password, enabled) VALUES (?, ?, 1)',
      [username, password]
    );
    
    // 2. Add to PAP secrets file
    const papSecretsPath = '/etc/ppp/pap-secrets';
    let content = '';
    
    if (fs.existsSync(papSecretsPath)) {
      content = fs.readFileSync(papSecretsPath, 'utf8');
      // Remove old entry if exists
      const lines = content.split('\n').filter(line => !line.includes(`${username} `));
      content = lines.join('\n') + '\n';
    }
    
    // Add new entry: username * password *
    content += `${username} * ${password} *\n`;
    fs.writeFileSync(papSecretsPath, content);
    
    // 3. Set proper permissions
    await execPromise(`chmod 600 ${papSecretsPath}`);
    
    console.log(`[PPPoE-Server] User ${username} added successfully`);
    return { success: true };
...
async function deletePPPoEUser(userId) {
  ...
  try {
    ...
    const user = await db.get('SELECT username FROM pppoe_users WHERE id = ?', [userId]);
    ...
    // 3. Remove from PAP secrets
    const papSecretsPath = '/etc/ppp/pap-secrets';
    if (fs.existsSync(papSecretsPath)) {
      const content = fs.readFileSync(papSecretsPath, 'utf8');
      const lines = content.split('\n').filter(line => !line.includes(`${username} `));
      fs.writeFileSync(papSecretsPath, lines.join('\n'));
    }
    
    console.log(`[PPPoE-Server] User ${username} deleted successfully`);
    return { success: true };
...
async function updatePPPoEUser(userId, updates) {
  try {
    ...
    // Update PAP secrets if password changed
    if (password !== undefined) {
      const finalUsername = username || currentUser.username;
      const papSecretsPath = '/etc/ppp/pap-secrets';
      
      if (fs.existsSync(papSecretsPath)) {
        let content = fs.readFileSync(papSecretsPath, 'utf8');
        const lines = content.split('\n').filter(line => !line.includes(`${currentUser.username} `));
