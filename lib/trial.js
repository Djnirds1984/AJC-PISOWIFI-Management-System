...
async function checkTrialStatus(hardwareId, cloudStatus = null) {
  try {
    ...
    // If revoked (either locally or reported by cloud), no trial
    const isRevoked = (licenseInfo && licenseInfo.is_revoked) || (cloudStatus && cloudStatus.isRevoked);
    const hasHadLicense = (licenseInfo && licenseInfo.license_key) || (cloudStatus && cloudStatus.licenseKey);

    if (isRevoked || hasHadLicense) {
      // Ensure local DB is updated if cloud reported revocation
      if (isRevoked && licenseInfo && !licenseInfo.is_revoked) {
      ...
      return {
        ...
        trialEnded: true,
        isRevoked: true,
        hasHadLicense: Boolean(hasHadLicense),
        daysRemaining: 0,
        expiresAt: null
      };
    }
  ...
  try {
    await db.run(
      `INSERT INTO license_info (hardware_id, license_key, is_active, activated_at) 
       ...
       license_key = ?, is_active = 1, activated_at = ?`,
