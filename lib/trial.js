 ...
 */
async function checkTrialStatus(hardwareId) {
  try {
    ...
    // If revoked or has had a license before, no trial
    if (licenseInfo && (licenseInfo.is_revoked || licenseInfo.license_key)) {
      return {
        isTrialActive: false,
        trialEnded: true,
        isRevoked: Boolean(licenseInfo.is_revoked),
        hasHadLicense: Boolean(licenseInfo.license_key),
        daysRemaining: 0,
        expiresAt: null
      };
    }
