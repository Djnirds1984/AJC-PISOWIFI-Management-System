...
class WifiSync {
  ...
  async syncDeviceToCloud(deviceData) {
    ...
    try {
      ...
      // Check if session_token already exists in cloud (to prevent duplicates)
      if (deviceData.session_token) {
        const { data: existingByToken } = await this.supabase
          .from('wifi_devices')
          .select('id, machine_id, mac_address, remaining_seconds')
          .eq('session_token', deviceData.session_token)
          .maybeSingle();
          
        if (existingByToken) {
          console.log(`[WifiSync] Session token ${deviceData.session_token} already exists on machine ${existingByToken.machine_id}`);
          
          // If it's our machine, update it
          if (existingByToken.machine_id === this.machineId) {
            console.log(`[WifiSync] Updating existing record for our machine`);
            ...
            console.log(`[WifiSync] Updated existing device with session token ${deviceData.session_token}`);
            return { existingDevice: data, updated: true };
          } else {
            // Session belongs to another machine, don't duplicate
            console.log(`[WifiSync] Session belongs to another machine, skipping duplicate`);
            return { existingDevice: existingByToken, updated: false };
          }
        }
      }
      ...
      if (existingDevice) {
        ...
        // Use UPSERT with conflict resolution on session_token
        ...
        console.log(`[WifiSync] Upserting device data:`, {
        ...
        if (error) {
          console.error(`[WifiSync] Upsert failed for ${deviceData.mac_address}:`, error.message);
          // Even on error, don't queue if it's a duplicate key error
          if (error.code === '23505') { // duplicate key violation
            console.log(`[WifiSync] Duplicate key error - session likely already exists, not queuing`);
            return { existingDevice: null, updated: false };
          }
          throw error;
        }
        result = data;
        
        console.log(`[WifiSync] Successfully upserted device ${deviceData.mac_address} to cloud`);
      }

      return returnData;
    } catch (err) {
      console.error(`[WifiSync] Failed to sync device ${deviceData.mac_address}:`, err.message);
      // Only queue if it's not a duplicate key error
      if (err.code !== '23505') {
  ...
  queueSync(type, data) {
    ...
    if (type === 'device' && data.session_token) {
      const existingIndex = this.queue.findIndex(item => 
        item.type === 'device' && 
        item.data.session_token === data.session_token
      );
      
      if (existingIndex !== -1) {
        console.log(`[WifiSync] Skipping duplicate queue entry for session ${data.session_token}`);
        return;
      }
    }
    
    // Also prevent duplicate MAC addresses for device type
    if (type === 'device' && data.mac_address) {
      const existingMacIndex = this.queue.findIndex(item => 
        item.type === 'device' && 
        item.data.mac_address === data.mac_address
      );
      
      if (existingMacIndex !== -1) {
        console.log(`[WifiSync] Skipping duplicate queue entry for MAC ${data.mac_address}`);
        return;
      }
    }
  ...
  async processQueue() {
    ...
    console.log(`[WifiSync] Processing queue (size: ${this.queue.length})`);
    
    const itemsToProcess = [...this.queue];
    this.queue = []; // Clear queue immediately to prevent buildup
    this.saveQueue(); // Save empty queue immediately
    ...
    for (const item of itemsToProcess) {
      ...
      try {
        if (item.type === 'device') {
          const result = await this.syncDeviceToCloud(item.data);
          // Consider it successful if we got a result (even if not updated)
          success = result !== null;
          // If result is null but no error was thrown, it was likely skipped due to duplicate
          if (result === null) {
            skipped = true;
            skippedCount++;
          } else {
        ...
        console.log(`[WifiSync] Queue item failed:`, e.message);
      ...
      // Log progress
      if (processedCount % 5 === 0 || processedCount === itemsToProcess.length) {
        console.log(`[WifiSync] Queue progress: ${processedCount}/${itemsToProcess.length} processed (${successCount} success, ${skippedCount} skipped)`);
      }
    }
    ...
    if (this.queue.length > 0) {
      this.saveQueue();
      console.log(`[WifiSync] Queue saved with ${this.queue.length} remaining items`);
    } else {
      console.log(`[WifiSync] Queue processing complete - all items handled`);
    }
  }
