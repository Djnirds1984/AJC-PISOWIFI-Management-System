...
class WifiSync {
  ...
  async syncAllDevices() {
    ...
    try {
      ...
      for (const device of localDevices) {
        ...
        // Log devices from other machines
        if (existingDevice && existingDevice.machine_id !== this.machineId) {
          console.log(`[WifiSync] Monitoring device ${device.mac} from remote machine ${existingDevice.machine_id}`);
        }
  ...
  async checkDeviceSession(macAddress) {
    ...
    try {
      ...
      if (data && data.machine_id !== this.machineId) {
