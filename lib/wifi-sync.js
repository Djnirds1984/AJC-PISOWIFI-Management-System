...
class WifiSync {
  ...
  async syncAllDevices() {
    ...
    try {
      ...
      const sessions = await db.all(`
        SELECT mac, ip, remaining_seconds, total_paid, session_token
        FROM sessions 
      ...
      for (const device of localDevices) {
        ...
        const cloudDeviceData = {
          ...
          session_start_time: session ? new Date(session.connected_at).toISOString() : null,
          session_duration_seconds: session ? Math.floor((Date.now() - session.connected_at) / 1000) : 0,
          ...
          last_heartbeat: new Date(device.last_seen).toISOString(),
          last_sync_attempt: new Date().toISOString(),
          sync_status: 'pending',
          allowed_machines: null, // Will be populated by cross-machine logic
          created_at: new Date(device.connected_at).toISOString(),
  ...
  async syncDeviceToCloud(deviceData) {
    ...
    try {
      // Check if device already exists in cloud
      const { data: existingDevice, error: fetchError } = await this.supabase
        .from('wifi_devices')
        .select('id, session_token, remaining_seconds')
      ...
      if (existingDevice) {
        ...
        // Preserve session continuity - only update if local data is newer
        const shouldUpdate = !existingDevice.session_token || 
                            deviceData.session_token ||
                            deviceData.last_heartbeat > existingDevice.updated_at;
        ...
        if (deviceData.session_token) {
          deviceData.session_token = deviceData.session_token || `sess_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        }
