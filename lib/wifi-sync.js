...
class WifiSync {
  ...
  async checkDeviceSession(macAddress) {
    ...
    try {
      console.log(`[WifiSync] Executing query for MAC: ${macAddress}`);
      ...
      console.log(`[WifiSync] Test query result:`, testQuery);
      ...
      if (error) {
        console.error(`[WifiSync] Query error for ${macAddress}:`, error);
        // Try a simpler query as fallback
        try {
          console.log(`[WifiSync] Trying simplified query...`);
          const simpleData = await this.supabase
            .from('wifi_devices')
            .select('*')
            .eq('mac_address', macAddress);
          console.log(`[WifiSync] Simplified query result:`, simpleData);
        } catch (simpleErr) {
          console.error(`[WifiSync] Simplified query also failed:`, simpleErr);
        }
        throw error;
      }
      
      console.log(`[WifiSync] Query result for ${macAddress}:`, data ? 'FOUND' : 'NOT FOUND');
      if (data) {
        console.log(`[WifiSync] Session data:`, JSON.stringify(data, null, 2));
        console.log(`[WifiSync] Current machine ID: ${this.machineId}`);
        console.log(`[WifiSync] Session machine ID: ${data.machine_id}`);
        console.log(`[WifiSync] Is connected: ${data.is_connected}`);
      }
      ...
      if (data && data.machine_id && data.machine_id !== this.machineId) {
        ...
        console.log(`[WifiSync] Query conditions - MAC: ${macAddress}, Connected: true`);
      }
